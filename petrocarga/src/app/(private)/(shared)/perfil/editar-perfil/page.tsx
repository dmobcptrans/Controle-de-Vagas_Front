 ...
 * @version 2.0.0
 *
 * @description Página de edição de perfil universal.
 * Funciona para MOTORISTA, AGENTE e GESTOR (formulário completo) e ADMIN
 * (aviso de indisponibilidade até que exista um formulário próprio), usando
 * `user.permissao` (vindo do useAuth) para decidir qual API/form usar.
 *
 * ----------------------------------------------------------------------------
 * 📋 FLUXO COMPLETO:
 * ----------------------------------------------------------------------------
 ...
 *    - PERSONA_CONFIG mapeia cada `permissao` para: função de busca e
 *      renderização do formulário de edição correspondente.
 *    - PERSONA_TEMAS mapeia cada `permissao` para as cores/textos/ícones
 *      próprios daquela persona (mantendo a diferenciação visual que já
 *      existia para o motorista).
 *
 * 3. BUSCA DE DADOS:
 *    - useEffect dispara fetchDados na montagem, usando a config da persona
 *
 * 4. ESTADOS DE UI:
 *    a) LOADING INICIAL
 *    b) ERRO DE AUTENTICAÇÃO/BUSCA
 *    c) DADOS NÃO ENCONTRADOS
 *    d) SUCESSO — renderiza o formulário de edição da persona
 *
 * ----------------------------------------------------------------------------
 * 🧠 DECISÕES TÉCNICAS:
 * ----------------------------------------------------------------------------
 *
 * - TEMA POR PERSONA: cada persona mantém sua própria identidade visual
 *   (cor de gradiente, cor de botão, ícone de sucesso, textos do card
 *   informativo), definida em PERSONA_TEMAS.
 * - FORMULÁRIO POR PERSONA: PERSONA_CONFIG.renderForm decide qual
 *   componente de formulário (EditarMotorista/EditarAgente/EditarGestor)
 *   renderizar. Para ADMIN, como ainda não existe um formulário próprio,
 *   é exibido um aviso — quando o componente EditarAdmin existir, basta
 *   adicioná-lo na config.
 *
 * @see /components/motorista/editar/edicao-perfil.tsx
 * @see /components/agente/editar/edicao-perfil.tsx
 * @see /components/gestor/editar/edicao-perfil.tsx
...
type Dados = Motorista | Agente | Gestor;
...
interface PersonaConfig {
  ...
  // Retorna null quando a persona ainda não tem formulário próprio (ex: ADMIN)
  renderForm: (dados: Dados, onSuccess: () => void) => React.ReactNode | null;
}
...
// --------------------------------------------------------------------------
// CONFIGURAÇÃO POR PERSONA (dados + formulário)
// --------------------------------------------------------------------------
// Se a chave de retorno de alguma API for diferente do que está aqui,
// ou se o prop esperado pelo componente de formulário tiver outro nome,
// ajuste apenas o bloco correspondente abaixo.
const PERSONA_CONFIG: Record<Permissao, PersonaConfig> = {
  MOTORISTA: {
    fetchDados: async (userId) => {
      ...
      return { error: resultado.error, message: resultado.message, dados: resultado.motorista };
    },
    renderForm: (dados, onSuccess) => (
      <EditarMotorista motorista={dados as Motorista} onSuccess={onSuccess} />
    ),
  },
  AGENTE: {
    fetchDados: async (userId) => {
      const resultado = await getAgenteByUserId(userId);
      return { error: resultado.error, message: resultado.message, dados: resultado.agente };
    },
    renderForm: (dados, onSuccess) => (
      <EditarAgente agente={dados as Agente} onSuccess={onSuccess} />
    ),
  },
  GESTOR: {
    fetchDados: async (userId) => {
      const resultado = await getGestorByUserId(userId);
      return { error: resultado.error, message: resultado.message, dados: resultado.gestor };
    },
    renderForm: (dados, onSuccess) => (
      <EditarGestor gestor={dados as Gestor} onSuccess={onSuccess} />
    ),
  },
};
...
const PERSONA_TEMAS: Record<Permissao, PersonaTema> = {
  MOTORISTA: {
    label: 'motorista',
    corGradiente: 'from-orange-50/30 via-white to-yellow-50/30',
    corBotao: 'bg-orange-600 hover:bg-orange-700',
    corCardInfo: 'bg-orange-50 border-orange-100',
    corTextoInfo: 'text-orange-900',
    corTextoInfoDescricao: 'text-orange-700',
    corIconeInfo: 'text-orange-500',
    IconeSucesso: CheckCircle,
    corIconeSucesso: 'text-blue-600',
    corFundoIconeSucesso: 'from-blue-100 to-purple-100',
    tituloInfo: 'Informação importante para motoristas',
    descricaoInfo:
  ...
  AGENTE: {
    label: 'agente',
    corGradiente: 'from-emerald-50/30 via-white to-teal-50/30',
    corBotao: 'bg-emerald-600 hover:bg-emerald-700',
    corCardInfo: 'bg-emerald-50 border-emerald-100',
    corTextoInfo: 'text-emerald-900',
    corTextoInfoDescricao: 'text-emerald-700',
    corIconeInfo: 'text-emerald-500',
    IconeSucesso: CheckCircle,
    corIconeSucesso: 'text-emerald-600',
    corFundoIconeSucesso: 'from-emerald-100 to-teal-100',
    tituloInfo: 'Informação importante para agentes',
    descricaoInfo:
      'Mantenha seus dados de contato sempre atualizados para garantir a comunicação com motoristas e gestores.',
    descricaoPagina: 'Atualize suas informações de contato e atuação',
  },
  GESTOR: {
    label: 'gestor',
    corGradiente: 'from-purple-50/30 via-white to-indigo-50/30',
    corBotao: 'bg-purple-600 hover:bg-purple-700',
    corCardInfo: 'bg-purple-50 border-purple-100',
    corTextoInfo: 'text-purple-900',
    corTextoInfoDescricao: 'text-purple-700',
    corIconeInfo: 'text-purple-500',
    IconeSucesso: ShieldCheck,
    corIconeSucesso: 'text-purple-600',
    corFundoIconeSucesso: 'from-purple-100 to-indigo-100',
    tituloInfo: 'Informação importante para gestores',
    descricaoInfo:
      'Mantenha seus dados atualizados para garantir o acesso correto às ferramentas de gestão.',
    descricaoPagina: 'Atualize suas informações de gestão e contato',
  },
};
...
export default function EditarPerfilPage() {
  ...
  const tema = permissao ? PERSONA_TEMAS[permissao] : PERSONA_TEMAS.MOTORISTA;
  ...
  useEffect(() => {
    ...
    async function fetchDados() {
      ...
      try {
        ...
        if (resultado.error) {
          setError(resultado.message || `Erro ao buscar perfil de ${tema.label}.`);
          setDados(null);
        } else if (!resultado.dados) {
          setError(`Perfil de ${tema.label} não encontrado.`);
        } else {
          setDados(resultado.dados);
        }
      } catch {
        setError(`Erro ao buscar perfil de ${tema.label}.`);
      } finally {
        setLoading(false);
      }
    }
  ...
  const formulario = dados ? config?.renderForm(dados, () => router.push('/perfil')) : null;
      ...
      <div className="mx-auto px-3 xs:px-4 sm:px-5 lg:px-6 xl:px-8 py-4 sm:py-6 lg:py-8">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="mb-8 sm:mb-10 lg:mb-12 relative">
            {/* Fundo gradiente (cor da persona) */}
            <div
            ...
            <div className="relative">
              ...
              <div className="flex justify-between items-start mb-6 sm:mb-0">
                ...
                <div className="hidden sm:flex items-center gap-2 text-sm text-gray-500">
                  ...
                  <span>Editando perfil de {tema.label}</span>
                </div>
              </div>

              {/* Título e ícone */}
              <div className="text-center pt-4 sm:pt-8 lg:pt-12 pb-6 sm:pb-8">
                {/* Ícone decorativo (varia por persona) */}
          ...
          <div className="bg-white rounded-lg sm:rounded-xl shadow-sm border border-gray-200 overflow-hidden">
            {!dados || !formulario ? (
              /* PERFIL NÃO ENCONTRADO OU SEM FORMULÁRIO DISPONÍVEL PARA A PERSONA */
              ...
              /* FORMULÁRIO DE EDIÇÃO DA PERSONA */
              <div className="p-3 xs:p-4 sm:p-6 lg:p-8">{formulario}</div>
            )}
          </div>

          {/* CARD INFORMATIVO (cor da persona) */}
          <div
            className={`mt-4 xs:mt-5 sm:mt-6 md:mt-8 p-3 xs:p-4 sm:p-6 rounded-lg border ${tema.corCardInfo}`}
          >
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2 sm:gap-3">
              <div className="flex-shrink-0">
                <Info className={`w-4 h-4 sm:w-5 sm:h-5 md:w-6 md:h-6 ${tema.corIconeInfo}`} />
              </div>
