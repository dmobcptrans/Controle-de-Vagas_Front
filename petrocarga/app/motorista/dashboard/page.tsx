...
function StatusBadge({ status }: { status: StatusKey }) {
    ...
    return (
        <span className={`text-[10px] font-semibold px-2.5 py-1 rounded-full whitespace-nowrap ${className}`}>
            {label}
        </span>
    );
}
            ...
            <main className="px-4 sm:px-8 pb-16 max-w-4xl mx-auto">
                ...
                <div className="grid grid-cols-3 gap-2.5 mb-5">
                    {[
                        ...
                        >
                            <p className={`text-2xl font-bold ${accent ? "text-[#168821]" : "text-[#071D41]"}`}>
                                {loading ? "—" : num}
                            </p>
